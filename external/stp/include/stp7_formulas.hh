...
namespace stp
{
     ...
     * \date 2007
     *
     * This class is used for getting the correct values for the time inverals of
     * a 3rd order trajectory. It must be provided with sufficient information about
     * the type of profile:
     *      - TT / TW / WT / WW profile
     *      - with / without cruising phase
     *      - double deceleration?
     *      - start with deceleration since a0 > amax?
     ...
     * Calculate the time intervals associated with this root and check whether they
     * are a valid solution (e.g. no negative time intervals).
     *
     ...
     *
     * \warning The solutions for the times are given back as time intervals instead
     * of absolute time points. The index of the time array ranges from 1..7, so a
    ...
    public:
         ...
         */
        static void solveProfile(
            double t[8], std::string type, bool bCruise,
            bool bDoubleDec, bool bDecAcc, bool bMoveForward,
            double x0, double xTarget, double v0, double vmax,
            double a0, double amax, double jmax, double stretchToTime = 0) throw(std::logic_error);

        /// method returns true, if all values t[1..7] are positive or zero.
        static bool areValidTimeIntervalls(double t[8]);
    private:
        static void calcCoeffs(double coeffs[7], std::string type, bool bCruise,
                               bool bDoubleDec, bool bDecAcc, bool bMoveForward,
                               double x0, double xTarget, double v0, double vmax,
                               double a0, double amax, double jmax, double stretchToTime,
                               double t[8]);

        static void calcTimeIntervalls(double t[8], std::string type, bool bCruise,
                                       bool bDoubleDec, bool bDecAcc, bool bMoveForward,
