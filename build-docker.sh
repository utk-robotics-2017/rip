...
stdbuf -o0 docker build --tag utkrobotics/rip:$(git_branch_norm) . | \
  ...
  dialog --title "Building RIP Docker container" --gauge "Starting Docker Build..." 6 50 0

if (dialog --title "Start container?" --yesno "Start an interactive session in the docker container?" 8 68) then
