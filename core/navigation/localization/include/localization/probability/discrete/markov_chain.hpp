        ...
        namespace localization
        {
            namespace probability
            {
                ...
                public:
                    ...
                    {
                        std::vector<X> result;
                        result.reserve(m_range.size());
                        for (auto& pair : m_range)
                        {
                    ...
                    {
                        std::vector<X> result;
                        result.reserve(m_transitions.size());
                    ...
                    {
                        auto end_map = m_transitions.find(start);
                        if (end_map == m_transitions.end())
                        {
                    ...
                    void set(const X& end, const X& start, double p)
                    {
                        ...
                        if (end_map == m_transitions.end())
                        {
                            end_map = m_transitions.emplace(start, std::map<X, double>()).first;
                        }
                    ...
                    std::shared_ptr<DistributionValueMap<X>> likelihoodOf(const X& data) const
                    {
                        ...
                        for (auto& end_map : m_transitions)
                        {
                    ...
                    std::shared_ptr<DistributionValueMap<X>> marginalize(const RandomDistribution<X>& start) const
                    {
                        auto result = std::make_shared<DistributionValueMap<X>>();
                        for (auto& xfr_map : m_transitions)
                        {
                    ...
                    std::shared_ptr<DistributionValueMap<X>> bayesianInference(const X& data, const DistributionValueMap<X>& prior) const
                    {
                        auto likelihood = likelihoodOf(data);
                        return likelihood->product(prior);
                    }
                private:
                    std::map<X, std::map<X, double>> m_transitions;
                    std::map<X, X> m_range;
                };
            }
        }
